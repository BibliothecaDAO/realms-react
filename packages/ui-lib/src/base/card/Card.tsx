...
export function Card({ children, className }: Props) {
  return (
    <div
      className={`${className} duration-150 transition-all  hover:bg-gray-600/80 flex flex-wrap px-4 pb-4 pt-2 text-gray-600 bg-gray-800/30 rounded shadow-inner overflow-hidden group border-off-200/40 border`}
    >
      {children}
    </div>
  );
...
export function CardTitle({ children, className }: Props) {
  return (
    <div className={`${className} z-20 w-full tracking-widest uppercase `}>
      <h5 className="px-2 py-1 font-semibold rounded border-off-200/40 text-white/90 bg-gradient-to-b from-gray-500/40 font-body">
        {children}
      </h5>
    </div>
  );
